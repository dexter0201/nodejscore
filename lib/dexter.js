...
NodeCore.prototype.serve = function (options, callback) {
    ...
    var database = mongoose.connect(defaultConfig.db || '', function (err) {
        if (err) {
            ...
            return console.error('**Could not connect to MongoDB. Please ensure mongod is running and restart Dexter app.**');
        }
        ...
        Dexter.config = new Config(function (err, config) {
            var app = require('./bootstrap')(options, database);
            var httpServer = http.createServer(app);

            Dexter.register('http', httpServer);
            ...
            if (config.https && config.https.port) {
                ...
                Dexter.register('https', httpsServer);
...
NodeCore.prototype.aggregated = function (ext, group, callback) {
...
NodeCore.prototype.rebuildAggregated = function () {
    sortAggregatedAssetByWeight();
};
...
NodeCore.prototype.Module = function (name) {
    ...
    if (nameCap !== name) {
        _modules[name] = _modules[nameCap];
        delete _modules[nameCap];
    }
    ...
    this.aggregateAsset = function (type, asset, options) {
        options = options || {};
        asset = options.inline ? asset : (options.absolute ? asset : path.join(_modules[this.name].source, this.name, 'public/assets/', type, asset));
...
function getModulePath(name, plus) {
    return path.join(process.cwd(), _modules[name].source, name.toLowerCase(), plus);
}
...
function findModules(callback) {
                        ...
                        fs.readFile(path.join(process.cwd(), source, file, 'package.json'), function (err, data) {
                            ...
                            if (data) {
                                try {
                                    ...
                                    if (json.dexter) {
                                        _modules[capitaliseFirstLetter(json.name)] = {
    ...
    q.all([
        ...
        searchSource('packages/core'),
        searchSource('packages/custom'),
        searchSource('packages/contrib')
    ]).done(function () {
...
function aggregate(ext, asset, options) {
    ...
    if (asset) {
        return options.inline ? addInlineCode(ext, asset) : readFiles(ext, path.join(process.cwd(), asset));
    }

    libs = false;
    _events.on('modulesFound', function () {
        for (name in _modules) {
            readFiles(ext, path.join(process.cwd(), _modules[name].source, name.toLowerCase(), 'public'));
        }
    });
    ...
    function pushAggregateData(ext, filename, data) {
        if (ext === 'js') {
            ...
            _aggregated[group][ext].weights[filename] = {
                weight: weight,
                data: Dexter.config.clean.debug ? code : ugly.code
            };
        } else {
            group = options.group || 'header';
    ...
    function addInlineCode(ext, data) {
        ...
        md5.update(data);
        hash = md5.digest('hex');
        pushAggregateData(ext, hash, data);
    }
        ...
        fs.readdir(filePath, function (err, files) {
            ...
            fs.readFile(filePath, function (fileErr, data) {
                ...
                if (!data) {
                    ...
                    pushAggregateData(ext, filename, data);
                }
            });
        });
    }
...
function sortByWeight(group, ext, callback) {
