...
function Config(callback) {
    ...
    loadSettings(NodeJsCore.Singleton.config, callback);
...
NodeJsCore.prototype.Menus = function (name) {
    console.log('NodeJsCore.prototype.Menus - name: ', name);
    var _option = {
        roles: [
            'annonymous'
        ],
    ...
    this.add = function (options) {
        ...
        if (NodeJsCore.menus[options.menu]) {
            NodeJsCore.menus[options.menu].push(_menu);
        } else {
            NodeJsCore.menus[options.menu] = [_menu];
        }
    ...
    this.get = function (options) {
        ...
        if (NodeJsCore.menus[options.menu] || options.defaultMenu) {
            var items = options.defaultMenu.concat(NodeJsCore.menus[options.menu]);
...
function Module(name) {
    this.name = name;
    this.menus = NodeJsCore.Singleton.menu;
...
function findModules(callback) {
    ...
    q.all([
        searchSource('nodeNodeJsCore.modules'),
