...
function onInstance(nodejscoreInstance, defer) {
    nodejscoreInstance.resolve('defaultConfig', onInstanceAndConfig.bind(null, defer, nodejscoreInstance));
}

function onInstanceAndConfig(defer, nodejscoreInstance, config, done) {
    ...
    database = mongoose.connect(defaultConfig.db || '', defaultConfig.dbOptions || {}, function (err) {
        ...
        q.allSettled(dbPromisses).then(databasesReady.bind(null, defer, database));
    });
}
